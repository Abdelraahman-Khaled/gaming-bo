...
const Search = () => {
  ...
  return (
    <div
      ref={outsideREF}
      className=" w-full flex relative z-50 group items-center gap-2 justify-between px-4 border border-white/20 rounded-xl md:w-[60%] lg:w-[40%] bg-white/10 backdrop-blur-md shadow-lg"
    >
      <input
        value={query}
        onChange={(e) => {
      ...
      <AnimatePresence>
        {(games?.data || isLoading) && active && (
          <MotionItem
            ...
            animate={{ height: "auto" }}
            exit={{ opacity: 0 }}
            className="absolute w-full top-full z-[9999] bg-black/40 backdrop-blur-lg rounded-2xl shadow-xl border border-white/10 max-h-[40vh] overflow-y-scroll left-0 mt-2"
          >
            {isLoading ? (
